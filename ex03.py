...
intro = 'LAB 02 - Ex. 03\nWe are going to test and compare the algorithms we have implemented\nTraffic matrix values can be in the range [%s, %s] (with probability %s%%) or in the range [%s; %s] (otherwise)' % (high_traffic[0], high_traffic[1], p*100, low_traffic[0], low_traffic[1])
print(intro)

# Test use different values for N and delta
...
# TEST
# Open the output file (write mode), in which I'll print obtained results
		...
		for n in ns:
			for delta in deltas:
				...
				for s in range(n_simulations):
					...
					traffic_matrix = tm.random_TM_2(n, low_traffic[0], low_traffic[1], high_traffic[0], high_traffic[1], p)
					# Experiment number
